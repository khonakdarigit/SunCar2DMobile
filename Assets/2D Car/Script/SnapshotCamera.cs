...
#pragma warning restore 0649
    ...
    public static FileInfo SavePNG(byte[] bytes, string filename = "", string directory = "")
    {
        ...
        directory = directory != "" ? Directory.CreateDirectory(directory).FullName : Directory.CreateDirectory(Path.Combine(Application.persistentDataPath, "../Assets/Resources/Snapshots")).FullName;
    ...
    public Texture2D TakeObjectSnapshot(GameObject gameObject, int width = 700, int height = 550)
    {
        return TakeObjectSnapshot(gameObject, Color.clear, defaultPositionOffset,gameObject.transform.rotation, defaultScale, width, height);
    }
    ...
    /// <param name="height">The height of the snapshot image.</param>
    /// <returns>A Texture2D containing the captured snapshot.</returns>
    public Texture2D TakeObjectSnapshot(GameObject gameObject, Color backgroundColor, Vector3 positionOffset, Quaternion rotation, Vector3 scale, int width, int height)
    {
        if (gameObject == null)
